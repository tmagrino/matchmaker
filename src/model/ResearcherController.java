...
public class ResearcherController {
    ...
    public static void addHiddenStudent(EntityManager em, Researcher r,
                                        Student s) {
        EntityTransaction tx = em.getTransaction();
        if (r == null || s == null) {
            return;
    ...
    public static void removeHiddenStudent(EntityManager em, Researcher r,
                                           Student s) {
        EntityTransaction tx = em.getTransaction();
        if (r == null || s == null) {
    ...
    public static Researcher getResearcherByNetID(EntityManager em,
                                                  String netid) {
        EntityTransaction tx = em.getTransaction();

        String query = "select r from Researcher r where r.netID = \"" + netid +
                       "\"";
        List<Researcher> mylist = (List<Researcher>) em.createQuery(query)
                                                       .getResultList();
        try {
            return mylist.get(0);
        } catch (Exception e) {
    ...
    public static Researcher getResearcherByName(EntityManager em,
                                                 String name) {
        EntityTransaction tx = em.getTransaction();
    ...
    public static void editWebpage(EntityManager em, Researcher r,
                                   String webpage) {
        EntityTransaction tx = em.getTransaction();
        if (r == null) {
            return;
    ...
    public static void editDepartments(EntityManager em, Researcher r,
                                       String ids) throws InstantiationException,
        ...
        for (String id : idList) {
            if (id.length() > 0) {
                try {
                    ...
                    addDepartment(em, r,
                                  (Department) FieldValueController.getFieldValueById(em,
                                                                                      Long.parseLong(id),
                                                                                      FieldFactory.DEPARTMENT));
                } catch (NumberFormatException e) {
                    addDepartment(em, r,
                                  (Department) FieldValueController.createFieldValue(em,
                                                                                     id,
                                                                                     FieldFactory.DEPARTMENT));
                }
            }
    ...
    private static void addDepartment(EntityManager em, Researcher r,
                                      Department dep) {
        EntityTransaction tx = em.getTransaction();
        if (r == null) {
            return;
    ...
    public static void editArea(EntityManager em, Researcher r,
                                String ids) throws InstantiationException,
                                            IllegalAccessException {
        if (r == null) {
            return;
        ...
        for (String id : idList) {
            if (id.length() > 0) {
                try {
                    addArea(em, r,
                            (Interest) FieldValueController.getFieldValueById(em,
                                                                              Long.parseLong(id),
                                                                              FieldFactory.INTEREST));
                } catch (NumberFormatException e) {

                    addArea(em, r,
                            (Interest) FieldValueController.createFieldValue(em,
                                                                             id,
                                                                             FieldFactory.INTEREST));
                }
            }

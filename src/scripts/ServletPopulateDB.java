...
import model.FieldValueController;
...
import model.ResearcherController;
import model.Skill;
...
import model.Year;
import model.YearController;

public class ServletPopulateDB implements ServletContextListener {
    ...
    public static void populateDB(ServletContextEvent arg0) {
        ServletContext context = arg0.getServletContext();
        Scanner in;
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("test");
        EntityManager em = emf.createEntityManager();
        // Majors
        if (FieldValueController.getListOfType(em, FieldFactory.MAJOR)
                                .size() == 0) {
            try {
                ...
                while (in.hasNextLine()) {
                    ...
                    FieldValueController.createFieldValue(em, str,
                                                          FieldFactory.MAJOR);
                }
            } catch (Exception e) {
        ...
        // Minors
        if (FieldValueController.getListOfType(em, FieldFactory.MINOR)
                                .size() == 0) {
            try {
        ...
        // Colleges
        if (FieldValueController.getListOfType(em, FieldFactory.COLLEGE)
                                .size() == 0) {
            try {
        ...
        // Interests
        if (FieldValueController.getListOfType(em, FieldFactory.INTEREST)
                                .size() == 0) {
            try {
        ...
        // Skills
        if (FieldValueController.getListOfType(em, FieldFactory.SKILL)
                                .size() == 0) {
            try {
        ...
        // Departments
        if (FieldValueController.getListOfType(em, FieldFactory.DEPARTMENT)
                                .size() == 0) {
            try {
        ...
        if (StudentController.getAllStudents(em).size() == 0) {
            try {
                ...
                while (in.hasNextLine()) {
                    ...
                    for (String s : cols) {
                        colleges.add((College) FieldValueController.getItemByDescription(em,
                                                                                         s,
                                                                                         FieldFactory.COLLEGE));
                    }
                    // Majors
                    ...
                    for (String s : majs) {
                        majors.add((Major) FieldValueController.getItemByDescription(em,
                                                                                     s,
                                                                                     FieldFactory.MAJOR));
                    }
                    // Minors
                    ...
                    for (String s : mins) {
                        minors.add((Minor) FieldValueController.getItemByDescription(em,
                                                                                     s,
                                                                                     FieldFactory.MINOR));
                    }
                    // Skills
                    ...
                    for (String s : skls) {
                        skills.add((Skill) FieldValueController.getItemByDescription(em,
                                                                                     s,
                                                                                     FieldFactory.SKILL));
                    }
                    // Interests
                    ...
                    for (String s : ints) {
                        interests.add((Interest) FieldValueController.getItemByDescription(em,
                                                                                           s,
                                                                                           FieldFactory.INTEREST));
                    }
                    ...
                    User user = UserController.createUser(em, name, email,
                                                          netid);
                    Student s = StudentController.createStudent(em, name, netid,
                                                                gpa,
                                                                email, year,
                                                                colleges,
                                                                majors, minors,
        ...
        if (ResearcherController.getResearcherList(em).size() == 0) {
            try {
                ...
                while (in.hasNextLine()) {
                    ...
                    for (String s : deps) {
                        departments.add((Department) FieldValueController.getItemByDescription(em,
                                                                                               s,
                                                                                               FieldFactory.DEPARTMENT));
                    }
                    String webpage = researcherStrings[4];
                    Interest researchArea = (Interest) FieldValueController.getItemByDescription(em,
                                                                                                 "Machine Learning",
                                                                                                 FieldFactory.INTEREST);

                    User user = UserController.createUser(em, name, email,
                                                          netID);
                    Researcher r = ResearcherController.createResearcher(em,
                                                                         name,
                                                                         netID,
                                                                         email,
                                                                         departments,
        ...
        // String url, Researcher researcher, List<Interest> area, List<Skill>
        // skills
        if (ProjectController.getProjectList(em).size() == 0) {
            try {
                ...
                while (in.hasNextLine()) {
                    ...
                    for (String s : ress) {
                        researchers.add((Researcher) ResearcherController.getResearcherByNetID(em,
                                                                                               s));
                    }
                    ...
                    Project p = ProjectController.createProject(em, name, desc,
                                                                url,
                                                                researchers,
                                                                interests,
                                                                skills);

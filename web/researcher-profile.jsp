...
<div class="content">
	<!-- <h1>My Profile</h1> -->
	<div id="all-department" class="hidden"
		title="All Department Suggestions"
	></div>
	<div id="all-research_area" class="hidden"
		title="All Research Area Suggestions"
	></div>
	<div class="photo-info clearfix">
		<form name="profile" action="save-researcher-profile-changes.jsp"
			method="GET"
		>
			<p class="error-msg">Errors were found. Please correct the errors
				before saving.</p>
			<table class="info">
				...
				<tr <%= req_attr.contains(attr) ? "class=\"required\"" : "" %>>
					...
					<td class="field">
						...
						<p
							class="read-only <%= r.getAttribute(attr).equals("") ? "hidden" : "" %>"
						>
							<% if (attr == "URL"){%>
							...
							<%} %>
							<a
								class="edit-btn <%=(r.getAttribute(attr).length() > 80) ? "extended" : "" %>"
								href="#"
							> <img src="images/pencil_small.png" alt="edit" />
							</a>
						</p>
						<p
							class="editable <%= !r.getAttribute(attr).equals("") ? "hidden" : "" %>"
						>

...
%>
	...
	<form action="latestAdditions.jsp">
		Category:<br /> <select name="mydropdown"
			onchange="this.form.submit()"
		>
			<option value="recents" <% if(category.equals("recents")){%> selected
				<%} %>
			>All</option>
			<option value="college" <% if(category.equals("college")){%> selected
				<%} %>
			>Colleges</option>
			<option value="department" <% if(category.equals("department")){%>
				selected <%} %>
			>Departments</option>
			<option value="interest" <% if(category.equals("interest")){%>
				selected <%} %>
			>Interests</option>
			<option value="major" <% if(category.equals("major")){%> selected
				<%} %>
			>Majors</option>
			<option value="minor" <% if(category.equals("minor")){%> selected
				<%} %>
			>Minors</option>
			<option value="skill" <% if(category.equals("skill")){%> selected
				<%} %>
			>Skills</option>
		</select>
	</form>

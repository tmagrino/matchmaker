...
<div class="content">
  ...
  <p>
    <span class="hidden-message">Displaying hidden students</span><a
      class="filter-button hide"
      href="invite-students.jsp?showhidden=no"
    > Hide Students</a>
  </p>
  <%
		}
  ...
  <table class="project-list searchable">
    ...
    <tr>
      ...
      >Hide</a> <%
               	...
               	%> <% if(has_avail_proj){ %>
        <div id="invite-form-<%=s.getNetID() %>"
          class="invite-form hidden" title="Invite Students"
        >
          <form method="get" action="send-invitation.jsp">
            <p>Select which project you would like to invite the
              student to:</p>
            <% for(Project p: projs){ 
								if(ApplicationController.getApplication(em, s, p) == null){ %>
            ...
            <input type="hidden" name="stud-id"> <input
